...
eventListener.onEasyrtcMsg = function(connectionObj, msg, socketCallback, next) {
    ...
    //store to memory
    pub.util.logDebug("[Call StoreMessage] " + msg.targetRoom + ":" + msg.msgData);

    var roomId = msg.targetRoom;
    var username = connectionObj.getUsername();
    var message = msg.msgData;
    //store to memory
    if (!chatHistories.has(roomId)) {
        chatHistories.set(roomId, []);
    }
    chatHistories.get(roomId).push(username + ":" + message);
...
eventListener.onRoomJoin = function(connectionObj, roomName, roomParameter, callback) {
    ...
    pub.util.logDebug("[ONROOMJOIN]["+roomName+"] chatHistory: ", chatHistory);
    if(chatHistory){
        //pub.events.emit("emitEasyrtcCmd", connectionObj, "chatHistory", chatHistory, function(){});
    }  
